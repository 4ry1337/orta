...
use crate::{
    config::{
        ...
        COOKIE_AUTH_CSRF_STATE, COOKIE_SALT, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, HOST,
...
impl OAuthClient for GoogleOAuthClient {
    fn build(&self) -> Self {
        ...
        let redirect_url = RedirectUrl::new(format!("{}/api/auth/ggogle/callback", HOST))
    ...
    async fn callback(
        ...
        let (Some(csrf_state), Some(code_verifier)) = (stored_state, stored_code_verifier) else {
            warn!(name: "OAUTH","csrf state and code are not specified");
            return (StatusCode::BAD_REQUEST).into_response();
        };
        if csrf_state.value() != state {
            warn!(name: "OAUTH","csrf state is invalid");
            return (StatusCode::BAD_REQUEST).into_response();
        }
        ...
        let google_user = match Client::new()
            ...
            .await
        {
            Ok(response) => match response.json::<GoogleUser>().await {
                Ok(user) => user,
                Err(error) => {
                    error!(name: "OAUTH","unable to parse user from google:\n{}", error);
                    return (StatusCode::INTERNAL_SERVER_ERROR, "Something went wrong")
                        .into_response();
                }
            },
            Err(error) => {
                error!(name: "OAUTH","unable to get response from google:\n{}", error);
                return (StatusCode::BAD_REQUEST, "Something went wrong").into_response();
            }
        };
